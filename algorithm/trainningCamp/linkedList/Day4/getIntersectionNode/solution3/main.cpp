...
//寻找两个链表的交点
//思路：先考虑两个长度相同的链表，那么交点距离两个头指针的距离一定是相等的，只需要从头开始同步遍历即可
//现在两个链表长度不一定相等，那么只需要测出长度，然后求差值，让长的链表的头指针后移即可转化为等长的

//可以发现在链表的操作中，设置两个指针，然后预先移动一个指针，让两个指针对齐或者是给一个固定的差距是常见的技巧
